...
contract SemaphoreVerifier {
    ...
    function verifyProof(
        ...
        assembly {
            ...
            function checkPairing(pA, pB, pC, pubSignals, pMems, vkPoints) -> isOk {
                let _pPairing := add(pMems, pPairing)
                let _pVk := add(pMems, pVk)
                ...
                g1_mulAccC(_pVk, mload(add(vkPoints, 192)), mload(add(vkPoints, 224)), mload(add(pubSignals, 0)))

                g1_mulAccC(
                    _pVk,
                    mload(add(vkPoints, 256)),
                    mload(add(vkPoints, 288)),
                    mload(add(pubSignals, 32))
                )
                ...
                // vk_x
                mstore(add(_pPairing, 384), mload(add(pMems, pVk)))
                mstore(add(_pPairing, 416), mload(add(pMems, add(pVk, 32))))
